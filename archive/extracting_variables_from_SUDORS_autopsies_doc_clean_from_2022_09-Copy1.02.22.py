...
# # Extracting redcap Variables from SUDORS Autopsies from doc_clean from 2022 09.02.22
...
# ### Keep only the 2022 data since this is the scope of the present script

# In[8]:


narr_df = narr_df[narr_df.year == "2022"]


# ## Lists of terms associated with SUDORS variables
# 
# ## Added lists of paraphernalia-specific terms 
# 
...
var_names = ["forensic_center_bigrams", "history_of_drug_use_terms_bigrams", "other_substance_abuse_bigrams", 
                ...
                "hasevidenceofillicittar", "hasevidenceofillicitcrystal", "hasevidenceofillicitpackage", # "_185_relationship_status_",
                 "medhx_COPD", "medhx_asthma", "medhx_apnea",
...
terms_match(rep = narr_df.doc_clean, term_list = medhx_obesity)
...
narr_df.forensic_center_match.sum()
...
narr_df.other_substance_abuse.sum()
...
narr_df["history_of_drug_use"] = narr_df['doc_clean'].apply(lambda x: terms_match(x, term_list = term_list))


# In[64]:


narr_df.history_of_drug_use.sum()
...
narr_df["history_of_alcohol_abuse"] = narr_df['doc_clean'].apply(lambda x: terms_match(x, term_list = term_list))

print(sum(narr_df["history_of_alcohol_abuse"]))
...
narr_df["mental_health_condition"] = narr_df['doc_clean'].apply(lambda x: terms_match(x, term_list = term_list))


# In[67]:


narr_df.mental_health_condition.sum()
...
print(narr_df.cme_suicidethoughthistory.sum())
...
narr_df["hasevidenceofinjectionneedle"] = narr_df['full_narr'].apply(lambda x: terms_match(x, term_list = term_list))
...
narr_df["snortingpowdernose"] = narr_df['full_narr'].apply(lambda x: terms_match(x, term_list = term_list))
...
narr_df["hasroutesmoking"] = narr_df['full_narr'].apply(lambda x: terms_match(x, term_list = term_list))
...
narr_df["medhx_backpain"] = narr_df['full_narr'].apply(lambda x: terms_match(x, term_list = term_list))
...
narr_df.reset_index(inplace=True)


# In[138]:


narr_df.to_feather('extracted_variables_from_SUDORS_autopsies_2022.feather') # I changed narrative to autopsies
# to indicate that we are searching through the entire document instead of just the narrative
...
narr_df.to_excel("extracted_variables_from_SUDORS_narratives_2022.xlsx")
# I changed narrative to autopsies
# to indicate that we are searching through the entire document instead of just the narrative
